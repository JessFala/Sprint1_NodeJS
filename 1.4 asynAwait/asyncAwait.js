...
const delayedVal = (pw) => {
    return new Promise((resolve, reject) => {
        if (pw === "hola") {
            setTimeout (() => {
                resolve("El Password és correcte. Benvingutx.");
        }, 2000);
        } else {
            reject("Password incorrecte.");
        }
    });
}
const displayVal = async (pw) => {
    try {
        console.log ("Si us plau, esperi. L'operació pot trigar uns segons ...");
...
/*Nivell 2 Ex 1 - Crea una funció que retorni 
el doble del número que se li passa com a paràmetre després de 2 segons. 
Crea una altra funció que rebi tres números i 
calculi la suma dels seus dobles usant la funció anterior.*/
